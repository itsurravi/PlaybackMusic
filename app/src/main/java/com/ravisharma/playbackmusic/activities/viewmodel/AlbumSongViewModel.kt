...
class AlbumSongViewModel : ViewModel() {
    ...
    private var albumSong: MutableLiveData<ArrayList<Song>> = MutableLiveData()

    fun getAlbumSongs(albumId: String, contentResolver: ContentResolver): MutableLiveData<ArrayList<Song>> {
        viewModelScope.launch(Dispatchers.Main) {
            albumSong.value = withContext(Dispatchers.IO) {
                getAlbumSongsList(albumId, contentResolver)
            }
        }
        return albumSong
    }

    private fun getAlbumSongsList(albumId: String, contentResolver: ContentResolver): ArrayList<Song> {
        val songList = java.util.ArrayList<Song>()
        val musicUri = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
        val musicCursor = contentResolver.query(musicUri, null,
                MediaStore.Audio.Media.ALBUM_ID + "=" + albumId/* + " AND " + MediaStore.Audio.Media.DATA + " NOT LIKE ? ", arrayOf("%Record%")*/, null, MediaStore.Audio.Media.TITLE + " ASC")
